...
class TestToFock:
    ...
    def test_tofock_from_a_bargmann_vacuum_state(self):
        ...
        vacuum_fock_with_int_shape = to_fock(vacuum_bargmann, shape=80)
        assert vacuum_fock_with_int_shape.array[0, 0, 0] == 1
        assert vacuum_fock_with_int_shape.array.shape[-1] == 80
        assert vacuum_fock_with_int_shape.array.shape == (1, 80, 80)

    def test_incompatible_shape(self):
        r"""Tests that the ValueError raises when the shape given are incompatible."""
        vacuum_bargmann = Bargmann(*vacuum_state_Abc(n_modes=2))
        with pytest.raises(ValueError):
            to_fock(vacuum_bargmann, shape=[50])
    ...
    def test_tofock_from_a_bargmann_coherent_state(self):
        ...
        assert math.allclose(
            coherent_fock_no_shape.array[0, 2],
            (0.3 + 1j * 0.1) / math.sqrt(2.0 + 0j) * coherent_fock_no_shape.array[0, 1],
        )
        assert coherent_fock_no_shape.array.shape[-1] == settings.AUTOCUTOFF_MAX_CUTOFF
        ...
        assert math.allclose(
            ...
            (0.2 + 1j * 0.1) * coherent_twomode_fock_no_shape.array[0, 0, 0],
        )
        assert math.allclose(
    ...
    def test_tofock_from_a_bargmann_displacement_gate(self):
        ...
        dgate_fock_with_shape = to_fock(dgate_bargmann, shape=[10, 10])
        assert math.allclose(
            dgate_fock_with_shape.array[0, 0, 0], math.exp(-0.5 * (0.3**2 + 0.1**2))
        )
        assert math.allclose(
            dgate_fock_with_shape.array[0, 1, 0],
            (0.3 + 1j * 0.1) * dgate_fock_with_shape.array[0, 0, 0],
        )
        assert math.allclose(
        ...
        assert math.allclose(
            ...
            (-0.3 + 1j * 0.1) * dgate_fock_with_shape.array[0, 1, 0]
            + dgate_fock_with_shape.array[0, 0, 0],
        )
        assert dgate_fock_with_shape.array.shape == (1, 10, 10)
    ...
    def test_tofock_from_a_bargmann_squeezing_gate(self):
        ...
        assert math.allclose(
            ...
            1 / math.cosh(0.3) * sgate_fock_with_shape.array[0, 0, 0],
        )
        assert math.allclose(
            sgate_fock_with_shape.array[0, 2, 1],
            0,
        )
        assert sgate_fock_with_shape.array.shape == (1, 8, 12)
    ...
    def test_tofock_from_a_bargmann_beamsplitter_gate(self):
        ...
        bsgate_fock_with_shape = to_fock(bsgate_bargmann, shape=[5, 4, 7, 3])
        V = np.array(
            [
        ...
        assert math.allclose(bsgate_fock_with_shape.array[0, 0, 0, 0, 0], 1.0)
        assert math.allclose(bsgate_fock_with_shape.array[0, 0, 1, 0, 0], 0.0)
        assert math.allclose(bsgate_fock_with_shape.array[0, 0, 0, 0, 1], 0.0)
        assert math.allclose(bsgate_fock_with_shape.array[0, 0, 1, 0, 1], V[1, 1])
        assert math.allclose(bsgate_fock_with_shape.array[0, 0, 1, 1, 0], V[1, 0])
        assert math.allclose(bsgate_fock_with_shape.array[0, 1, 0, 1, 0], V[0, 0])
        assert math.allclose(bsgate_fock_with_shape.array[0, 1, 0, 0, 1], V[0, 1])
        assert math.allclose(bsgate_fock_with_shape.array[0, 0, 3, 6, 1], 0.0)
        assert bsgate_fock_with_shape.array.shape == (1, 5, 4, 7, 3)
    ...
    def test_tofock_from_a_bargmann_attenuator_channel(self):
        ...
        assert attenuator_bargmann_fock_with_shape.array[0, 0, 0, 2, 0] == 0.0
        assert math.allclose(
